...
* Metadata is attached in a JP2 UUID box (0x7575 6964) as per XMP spec. A program will be made available to perform this operation on any JP2 file. The JP2 XML box can be then skipped, as AVM can incorporate the original FITS header (unparsed). Merging files into JPX streams handles JP2 UUID boxes similarly to XML boxes.
